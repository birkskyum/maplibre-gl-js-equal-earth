...
import {equalEarthTransition, projectAdaptiveEqualEarth, unprojectAdaptiveEqualEarth, type EqualEarthParameters} from './equal_earth_utils.ts';
...
export class EqualEarthCameraHelper extends MercatorCameraHelper {
    constructor(private readonly parameters: EqualEarthParameters = {}) { super(); }

    /** Fixed Equal Earth camera paths remain finite when starting or ending at a geographic pole. */
    protected projectForCamera(worldSize: number, location: LngLat): Point {
        return this.parameters.transition === false ? projectAdaptiveEqualEarth(location, 1, this.parameters.center).mult(worldSize)
            : super.projectForCamera(worldSize, new LngLat(location.lng, clamp(location.lat, -85.0511287798066, 85.0511287798066)));
    }

    protected unprojectForCamera(worldSize: number, point: Point): LngLat {
        return this.parameters.transition === false ? unprojectAdaptiveEqualEarth(point.div(worldSize), 1, this.parameters.center)
            : super.unprojectForCamera(worldSize, point);
    }
    ...
    cameraForBoxAndBearing(options: CameraForBoundsOptions, padding: PaddingOptions, bounds: LngLatBounds, bearing: number, tr: IReadonlyTransform): CameraForBoxAndBearingHandlerResult {
        ...
        for (let i = 0; i < 32; i++) {
            ...
            const {min, max} = projectedBounds(bounds, zoom, angle, this.parameters);
            const size = max.sub(min).mult(tr.tileSize * Math.pow(2, zoom));
            if (size.x <= width && size.y <= height) low = zoom;
            else high = zoom;
        }
        if (equalEarthTransition(low, this.parameters.transition) === 0) return super.cameraForBoxAndBearing(options, padding, bounds, bearing, tr);
        const {min, max} = projectedBounds(bounds, low, angle, this.parameters);
        ...
        return {center: unprojectAdaptiveEqualEarth(center, equalEarthTransition(low, this.parameters.transition), this.parameters.center), zoom: low, bearing};
    }
}

/** Samples curved meridians, including the equator where the world is widest. */
function projectedBounds(bounds: LngLatBounds, zoom: number, angle: number, parameters: EqualEarthParameters): {min: Point; max: Point} {
    const min = new Point(Infinity, Infinity);
    const max = new Point(-Infinity, -Infinity);
    const transition = equalEarthTransition(zoom, parameters.transition);
    ...
    for (const lng of [bounds.getWest(), bounds.getEast()]) {
        for (const lat of latitudes) {
            const point = projectAdaptiveEqualEarth(new LngLat(lng, lat), transition, parameters.center).rotate(-angle);
